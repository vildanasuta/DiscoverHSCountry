...
mixin DataFetcher {
  ...
  Future<List<Review>> getReviewByLocationId(int locationId) async {
    ...
    Future<List<Review>> getBestReviewsByOthersForUsersBestRatedLocation(int locationId, int touristId) async {
    final Uri uri = Uri.parse('${ApiConstants.baseUrl}/Review');
    final response = await http.get(uri);
    ...
    if (response.statusCode == 200) {
      ...
      final reviews = jsonData
          ...
          .where((review) => review.locationId == locationId && review.rate>4.0 && review.touristId!=touristId)
          .toList();
      return reviews;
    } else {
      throw Exception('Failed to load data');
  ...
  Future<List<Location>> fetchLocationsByCityId(int cityId) async {
    ...
    if (response.statusCode == 200) {
      ...
      final locations = jsonData
          ...
          .where((location) => location.cityId == cityId && location.isApproved==true) 
          .toList();
      return locations;
    } else {
      throw Exception('Failed to load data');
  ...
  final Uri uri = Uri.parse('${ApiConstants.baseUrl}/VisitedLocationImage');
    ...
    if (response.statusCode == 200) {
      ...
      final images = jsonData
          ...
          .where((visitedLocationImage) => visitedLocationImage.visitedLocationId == visitedLocationId)
          .toList();
      return images;
    } else {
      throw Exception('Failed to load data');
   ...
   final Uri uri = Uri.parse('${ApiConstants.baseUrl}/ReservationService');
    ...
    if (response.statusCode == 200) {
      ...
      final images = jsonData
          ...
          .where((reservationService) => reservationService.serviceId == serviceId)
          .toList();
      return images;
    } else {
      throw Exception('Failed to load data');
...
Future<LocationVisits?> getLocationVisitsByLocationIdAndTouristId(int locationId, int touristId) async {
