...
class ViewEventsForTAO extends StatefulWidget {
  User user;
  String userType;
  ViewEventsForTAO({Key? key, required this.user, required this.userType})
      : super(key: key);
...
class _ViewEventsForAdminState extends State<ViewEventsForTAO>
    with DataFetcher {
  List events = [];
  bool isLoading = true;
  ...
  void fetchEvents() async {
    try {
      events = await getEventsByTAO(widget.user.userId);
    } catch (e) {
      // Handle errors
    } finally {
      setState(() {
